...
const Footer = () => {
  ...
  const fetchClinics = async () => {
    try {
      ...
      // Group branches by state/region
      const groupedMap: { [key: string]: any[] } = {};
      
      branches.forEach((b: any) => {
        const stateName = b.state || "Telangana";
      ...
      console.error("Error fetching clinics for footer:", error);
    }
  };
    ...
    <footer className="bg-[#0a0a0a] text-white">
      <div className="container mx-auto px-4 py-16 space-y-16">
        ...
        <div className="grid md:grid-cols-4 gap-12">
          ...
          <div>
            <h4 className="font-bold font-heading mb-6 border-b border-white/10 pb-2 inline-block text-teal-light uppercase tracking-wider text-sm">{t('Quick Links')}</h4>
          ...
          <div>
            <h4 className="font-bold font-heading mb-6 border-b border-white/10 pb-2 inline-block text-teal-light uppercase tracking-wider text-sm">{t('Contact Info')}</h4>
            <div className="space-y-4 text-sm font-medium">
              <div className="flex items-center gap-3">
                <Phone className="w-5 h-5 text-teal-light flex-shrink-0" />
                <span>+91 90590 51906</span>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="w-5 h-5 text-teal-light flex-shrink-0" />
                    ...
                    {stateGrp.cities.map((city: any) => (
                      <li key={city.name}>
                        <Link 
                          ...
                          className="hover:text-teal-light transition-colors text-sm font-medium text-white/70"
                        >
                          {city.name}
                        </Link>
                      </li>
