...
export default function AddCampaignPage() {
  ...
  useEffect(() => {
    if (!locationIdParam) {
      toast({ title: "Error", description: "Location ID is missing", variant: "destructive" });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await campaignsApi.createCampaign({ ...formData, locationId: Number(locationIdParam) });
  ...
  return (
    <PrivateRoute modulePath="admin/telecaller/campaigns" action="create">
      <div className="p-6 space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => router.back()}>
            <ArrowLeft className="h-4 w-4 mr-2" /> Back
          </Button>
          <h1 className="text-3xl font-bold text-gray-900">Add Campaign</h1>
        </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="pt-4 flex justify-end">
                <Button type="submit" disabled={loading} className="bg-primary">
                  {loading ? "Saving..." : <><Save className="w-4 h-4 mr-2"/> Save Campaign</>}
                </Button>
              </div>
            </form>
          </CardContent>
