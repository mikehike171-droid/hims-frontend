...
import { Plus, Edit, Trash2, Globe, FileText, Info, Upload, X, ImageIcon, CheckCircle2, AlertCircle, Layout, ArrowLeft, Save, Activity, ImagePlus, ShieldCheck, Microscope, Layers } from "lucide-react"
...
export default function AboutContentPage() {
    ...
    try {
      ...
      if (response && response.urls) {
        setFormData((prev: any) => ({
          ...prev,
          image_urls: [...prev.image_urls, ...response.urls]
        }))
        toast({
          title: "Success",
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true)
      if (currentItem) {
        await settingsApi.updateAbout(currentItem.id, formData)
        toast({
          title: "Success",
          description: "About content updated successfully",
        })
      } else {
        await settingsApi.createAbout(formData)
        toast({
          title: "Success",
  ...
  return (
    <PrivateRoute modulePath="admin/website" action="view">
      <div className="p-8 space-y-8 bg-slate-50 min-h-screen">
        ...
        {/* --- PROFESSIONAL DASHBOARD DIALOG (Treatment Style) --- */}
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="max-w-7xl h-[95vh] flex flex-col p-0 border-none rounded-[2rem] overflow-hidden bg-slate-50 ring-1 ring-black/5 text-slate-900">
            
            {/* Immersive Header (Toolbar style) */}
               ...
               <div className="p-10 container mx-auto">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start pb-20">
                     ...
                     <div className="space-y-8">
                           ...
                           <CardContent className="p-8">
                              ...
                              <div className="mt-8 pt-8 border-t border-slate-50 space-y-4">
                                 <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                    ...
                                    <div className="flex items-center gap-3">
                                       <span className={`text-[9px] font-black uppercase tracking-widest ${formData.status === 'active' ? 'text-emerald-600' : 'text-slate-400'}`}>
                                          {formData.status === 'active' ? 'Live' : 'Hidden'}
                                       </span>
                                       <Switch
                        ...
                        {/* FINAL SAVE TRIGGER (Mobile/Floating style) */}
