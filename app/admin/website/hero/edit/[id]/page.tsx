...
export default function EditHeroSlidePage() {
  ...
  const fetchSlide = async () => {
    try {
      setFetching(true)
      const data = await settingsApi.getHeroSection(parseInt(id))
      if (data) {
        setFormData({
          title: data.title || "",
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true)
      await settingsApi.updateHeroSection(parseInt(id), formData)
      toast({
        title: "Success",
  ...
  if (fetching) {
    return <div className="p-10 text-center text-gray-500">Loading slide details...</div>
  }
            ...
            <Card>
              ...
              <CardContent className="space-y-4 pt-0">
                ...
                <Button onClick={handleSave} disabled={loading} className="w-full bg-primary h-12 text-[12px] font-black tracking-[0.1em] uppercase">
                  {loading ? "..." : <><Save className="h-4 w-4 mr-2" /> Update Slide</>}
                </Button>
              </CardContent>
            </Card>
          </div>
